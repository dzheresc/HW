...
├── clk_divider.v               # Clock divider (100 MHz to 25 MHz)
...
- Instantiates the clock divider to generate 25 MHz for `eth_ref_clk`
- Manages Ethernet PHY reset signal (`eth_rstn`)
- Connects all required ports as defined in the XDC constraints

### `clk_divider.v`
Clock divider module that:
- Divides the 100 MHz system clock (`CLK100MHZ`) by 4 to generate 25 MHz
- Uses a 2-bit counter with the MSB as the output clock
- Provides the `eth_ref_clk` signal required by the Ethernet PHY
...
| `eth_ref_clk` | Output | Ethernet reference clock (25 MHz, generated from CLK100MHZ) |
| `CLK100MHZ` | Input | System clock (100 MHz, used to generate eth_ref_clk) |

## How It Works

1. The 100 MHz system clock (`CLK100MHZ`) is divided by 4 using a clock divider to generate a 25 MHz clock for `eth_ref_clk`
2. The Ethernet PHY receives data packets on the physical Ethernet interface
3. The PHY converts the data to MII format and presents it on `eth_rxd[3:0]`
4. When valid data is available, `eth_rx_dv` is asserted
5. On each rising edge of `eth_rx_clk`, if `eth_rx_dv` is high, the module captures `eth_rxd[3:0]`
6. The captured data is immediately displayed on `led[3:0]`
7. LEDs hold the last valid data when `eth_rx_dv` is low
...
- The `eth_ref_clk` is generated by dividing the 100 MHz system clock by 4 using a clock divider module
